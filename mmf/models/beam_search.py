...
class BeamSearch:
    ...
    def init_batch(self, batch_dict, BOS_IDX):
        ...
        setattr(
            self,
            "top_k_scores",
            batch_dict["prev_inds"].new_zeros(
        ...
        for key in self.batch_dict_keys + ["prev_inds"]:
    ...
    def decode(self, batch_dict, t):
        ...
        # Build input index:
        batch_dict["prev_inds"] = self.add_next_word(
            batch_dict["prev_inds"], prev_position, new_position, t
        )
        ...
        if t + 1 < batch_dict["prev_inds"].shape[1]:
            self.completed_ids = self.find_complete_inds(
                batch_dict["prev_inds"], t + 1
            )
        else:
            # no beam got completed!
            self.completed_ids = torch.arange(batch_dict["prev_inds"].shape[0])

        finish = False
        if (len(self.completed_ids) == self._batch_size * self._decode_size) or (
            batch_dict["prev_inds"].shape[1] == t + 1
        ):
            # Save completed sequences!
            batch_dict["complete_seqs"] = batch_dict["prev_inds"][
                self.completed_ids, :
            ]
            finish = True
